...
LoginRoute.post('/', urlencodedParser, async function (req, res) {
    ...
    if(!username || !password)
        return res
        .status(400)
        .json({fullName: "",  username: username, password: "", role: false, status: false, message: 'Missing username/password'})
    
    try {
        const user = await User.findOne({username})
        if (!user)
            return res.status(400).json({fullName:"", username: username, password: "", role: false, status: false,message: "Invalid username"})
        
        // All good
        const unHashPass = await argon2.verify(user.password, password)
        if (!unHashPass){
            return res.status(400).json({fullName: "", username: username, password: "", role: false, status: false,message: "Incorrect password"})
        } else {
                req.session.isAuthenticated = true;
                req.session.userAuth = user
                // res.json({username, password: ""})
                res.status(200).json({fullName: user.fullName,  username: username, password: "", role: user.admin, status: true, message:"Login Successfully"})
               // res.redirect('./home')
        }
